...
informe caminhos explícitos de --env-file e RUNTIME_DIR para os arquivos privados existentes.
...
(cd tabelas/monday_sla_orcamento_globocorp && bash deploy/deploy.sh)
```
...
```bash
(cd tabelas/monday_sla_orcamento_globocorp && bash deploy/schedule.sh)
```
